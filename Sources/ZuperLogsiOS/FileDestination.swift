...
open class FileDestination: BaseDestination {
    ...
    public init(logFileURL: URL? = nil) {
        ...
        if let baseURL = baseURL {
            self.logFileURL = baseURL.appendingPathComponent("ZuperLogsiOS.Log", isDirectory: false)
        }
        super.init()
    }
    ...
    override open func send(_ level: ZuperLogsiOS.Level, msg: String, thread: String,
        ...
        let formattedString = super.send(level, msg: msg, thread: thread, file: file, function: function, line: line, payload: payload)
