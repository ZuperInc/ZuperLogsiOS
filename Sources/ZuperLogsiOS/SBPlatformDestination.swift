...
open class SBPlatformDestination: BaseDestination {
    ...
    public init(accessToken: String, serverURL: URL?, 
                region: String,
                appMode: String,
                entriesFileName: String = "zuper_entries.json",
                sendingfileName: String = "zuper_entries_sending.json",
                analyticsFileName: String = "zupe_analytics.json") {
        super.init()
    ...
    override open func send(_ level: ZuperLogsiOS.Level, msg: String, thread: String,
                            file: String, function: String, line: Int, payload: [String: Any]? = nil) -> String? {
...
//            "line": line]
        ...
        var functionPayload: [String: Any] = payload ?? [:]
        functionPayload["appMode"] = appMode
        ...
        let dict: [String: Any] = [
            "title": msg,
            "level": level.value,
            "channel": "mobile_v3_ios",
            "payload": functionPayload,
            "device_manufacturer": DEVICE_MODEL,
            "device_model": DEVICE_MODEL,
            "timestamp": getUTCLogServerDateFormatString(date: getLogServerDateFormatString(date: Date()) ?? ""),
            "app_build_number": appBuild(),
            "app_version": appVersion(),
            "os": "ios",
            "os_version": deviceDetailsDict["osVersion"] ?? ""
        ]
...
//                msg += "(\(str.length) chars) to server ..."
                toNSLog(msg)
                sendToServerAsync(logEntries) { ok, _ in
    ...
    func sendToServerAsync(_ str: [[String: Any]]?, complete: @escaping (_ ok: Bool, _ status: Int) -> Void) {
        ...
        guard !accessToken.isEmpty, !region.isEmpty, !(serverURL?.absoluteString ?? "").isEmpty else {
            toNSLog("Error! Could not initiate api call")
            return
        }
...
//            //print(request)
            ...
            // POST parameters
            let params: [String: Any] = ["region": region, "payload": payload]
    ...
    func sendingPointsForLevel(_ level: ZuperLogsiOS.Level) -> Int {
